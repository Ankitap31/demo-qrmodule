 ...
 */
class JsonResourceCliService implements JsonResourceCliServiceInterface {

  /**
   * A Drush 9.x command.
   *
   * @var \Drupal\json_resource\Commands\JsonResourceCommands
   */
  protected $command;
   ...
   *
   * @throws \Exception
   *   Throw exception if JsonResourceCommands method
   *   and drush function is not found.
   */
  ...
  public function json_resource_drush_command() {
    ...
    $items['json_resource-libraries-download'] = [
      'description' => 'Download third party libraries required by the Json Resource module.',
      'core' => ['8+'],
      'bootstrap' => DRUSH_BOOTSTRAP_DRUPAL_ROOT,
      'examples' => [
        'json_resource-libraries-download' => 'Download third party libraries required by the Json Resource module.',
      ],
      'aliases' => ['jrdl'],
    ];
    $items['json_resource-generate-commands'] = [
      'description' => 'Generate Drush commands from json_resource.drush.inc for Drush 8.x to JsonResourceCommands for Drush 9.x.',
      'core' => ['8+'],
      'bootstrap' => DRUSH_BOOTSTRAP_DRUPAL_SITE,
      'examples' => [
        'drush json_resource-generate-commands' => "Generate Drush commands from json_resource.drush.inc for Drush 8.x to JsonResourceCommands for Drush 9.x.",
      ],
      'aliases' => ['jrgc'],
    ];
  }
  ...
  public function drush_json_resource_libraries_download() {
    ...
    /** @var \Drupal\json_resource\JsonResourceLibrariesManagerInterface $libraries_manager */
    $libraries_manager = \Drupal::service('json_resource.libraries_manager');
  ...
  public function drush_json_resource_generate_commands() {
    // Drush 8.x.
    $commands = $this->drush_json_resource_generate_commands_drush8();
    $filepath = DRUPAL_ROOT . '/' . $this->drupal_get_path('module', 'json_resource') . '/drush/json_resource.drush.inc';
    file_put_contents($filepath, $commands);
    $this->drush_print("$filepath updated.");

    // Drush 9.x.
    $commands = $this->drush_json_resource_generate_commands_drush9();
    $filepath = DRUPAL_ROOT . '/' . $this->drupal_get_path('module', 'json_resource') . '/src/Commands/JsonResourceCommands.php';
    file_put_contents($filepath, $commands);
    $this->drush_print("$filepath updated.");
  }

  /**
   * Generate json_resource.drush.inc for Drush 8.x.
   *
   * @return string
   *   json_resource.drush.inc for Drush 8.x.
   *
   * @see drush/json_resource.drush.inc
   */
  protected function drush_json_resource_generate_commands_drush8() {
    $items = $this->json_resource_drush_command();
    $functions = [];
...
function $validate_hook() {
  return call_user_func_array([\Drupal::service('json_resource.cli_service'), '$validate_method'], func_get_args());
}";
      }
...
function $command_hook() {
  return call_user_func_array([\Drupal::service('json_resource.cli_service'), '$command_method'], func_get_args());
}";
      }
    }

    // Build commands.
    $commands = Variable::export($this->json_resource_drush_command());
 ...
 *
 * @see drush json_resource-generate-commands
 * @see \Drupal\json_resource\Commands\DrushCliServiceBase::generate_commands_drush8
 */

/**
 * Implements hook_drush_command().
 */
function json_resource_drush_command() {
  return $commands;
}
$functions
";
  ...
  /**
   * Generate JsonResourceCommands class for Drush 9.x.
   *
   * @return string
   *   JsonEditorCommands class for Drush 9.x.
   *
   * @see \Drupal\json_resource\Commands\JsonResourceCommands
   */
  protected function drush_json_resource_generate_commands_drush9() {
    $items = $this->json_resource_drush_command();
 ...
 *
 * @see drush json_resource-generate-commands
 * @see \Drupal\json_resource\Commands\DrushCliServiceBase::generate_commands_drush9
 */
namespace Drupal\json_resource\Commands;
